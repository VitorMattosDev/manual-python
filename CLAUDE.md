...
- **`\\` seguido de `[` no texto de um nó TikZ derruba a figura.** O LaTeX lê `\\[algo]` como quebra de linha com medida opcional. `[tool.ruff]\\[tool.pytest]` quebra com `There's no line here to end` — usar `\\{}[tool.pytest]`. Comum em figura que mostra seção de TOML/INI.
- **O cache do filtro TikZ (`~/.cache/tikz-diagram-filter/`) mascara figura quebrada.** Render local verde e CI vermelho sobre uma figura antiga é quase sempre isso: aqui o SVG vem do cache, no runner ele é recompilado. Antes de push que mexeu em figura, apagar a entrada do cache e renderizar de novo.
